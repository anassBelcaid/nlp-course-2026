...
The third notebook makes every important Transformer operation inspectable. You will implement:

- sinusoidal positional embeddings and their combination with token embeddings;
- causal multi-head self-attention in five separate stages;
- readable head reshaping with `einops`;
- the position-wise MLP;
- pre-norm residual Transformer blocks with `LayerNorm` and dropout;
- padded next-token loss and token error;
- a complete optimization loop; and
- temperature-controlled categorical sampling.

[Open Session 3 · Build a causal Transformer](starter/03_causal_transformer.ipynb){.btn .btn-primary}

[Download the Session 3 starter ZIP](starter/neural-sequence-lab-session-3.zip){.btn .btn-secondary download="neural-sequence-lab-session-3.zip"}

:::{.callout-important}
The attention exercise deliberately does not use `nn.MultiheadAttention`. Projection, head reshaping, score multiplication, masking, softmax, value retrieval, and output projection remain visible and are tested independently.
:::
...
The notebooks may also be uploaded directly to Google Colab.
